...
function getLowestDealPerStore(deals) {
  const best = {};
  deals.forEach(d => {
    if (!best[d.storeID] || parseFloat(d.price) < parseFloat(best[d.storeID].price)) {
      best[d.storeID] = d;
    }
  });
...
function showDeals(deals, title) {
  ...
  html += deals.map(d => {
    ...
    const savePct = parseFloat(d.savings).toFixed(0);
    return `<p><strong>${name}:</strong> $${price} (${savePct}%)</p>`;
  }).join('');
  cont.innerHTML = html;
...
function saveGame({ visitorId, title, store, price }) {
  fetch('/api/save_game', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ visitorId, title, store, price })
  })
